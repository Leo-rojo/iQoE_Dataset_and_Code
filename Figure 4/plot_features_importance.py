...
mae_all=np.load('save_mae_importance_for_all_hdtv.npy')
rmse_all=np.load('save_rmse_importance_for_all_hdtv.npy')
...
mean_importances=np.array(mae_all).mean(axis=0)
std_importances=np.array(mae_all).std(axis=0)
...
ax.set_ylim([0, 3])
plt.savefig('Plot/importance_hdtv_mae_mean_all.pdf', bbox_inches='tight')
plt.close()

#plot atypical #atypical users: [25  7] [ 0 28]
...
plt.xlabel('Shuffled influence factor', fontdict=font_axes_titles)
ax.set_ylim([0, 3])
plt.savefig('Plot/importance_hdtv_mae_mean_atypical.pdf', bbox_inches='tight')
plt.close()
...
df.to_csv('Plot/importance_hdtv_mae_mean_atypical.csv', sep=',', encoding='utf-8')
...
#barplot group importances with group std
#mean of group importances
mean_importances=np.array(rmse_all).mean(axis=0)
std_importances=np.array(rmse_all).std(axis=0)
...
plt.xlabel('Shuffled influence factor', fontdict=font_axes_titles)
ax.set_ylim([-0.5, 3])
plt.savefig('Plot/importance_hdtv_rmse_mean_all.pdf', bbox_inches='tight')
plt.close()
...
plt.xlabel('Shuffled influence factor', fontdict=font_axes_titles)
ax.set_ylim([0, 3])
plt.savefig('Plot/importance_hdtv_rmse_mean_atypical.pdf', bbox_inches='tight')
plt.close()

#write mean_importances in csv for atypical users for each legend
df = pd.DataFrame(mean_importances, index=legends, columns=['mean_importances'])
df.to_csv('Plot/importance_hdtv_rmse_mean_atypical.csv', sep=',', encoding='utf-8')
