...
def each_user(nr_chunks, rs, u, model, regr_choosen,n_queries,t_s):
    ...
    # all features
    synthetic_experiences = np.load('../output_data/exp_and_scores_each_ABR/7_chunks_exp_'+t_s+'.npy')
    scores_synthetic_users = np.load('../output_data/exp_and_scores_each_ABR/scores_scaled_'+t_s+'.npy')
    abrs.remove(t_s)
    abr_residual = abrs
    synth_exp1=np.load('../output_data/exp_and_scores_each_ABR/7_chunks_exp_'+abr_residual[0]+'.npy')
    scores_synth_1=np.load('../output_data/exp_and_scores_each_ABR/scores_scaled_'+abr_residual[0]+'.npy')
    synth_exp2 = np.load('../output_data/exp_and_scores_each_ABR/7_chunks_exp_' + abr_residual[1] + '.npy')
    scores_synth_2 = np.load('../output_data/exp_and_scores_each_ABR/scores_scaled_' + abr_residual[1] + '.npy')
    ...
    regressor_gsio = ActiveLearner(
        estimator=sklearn.svm.SVR(kernel = 'rbf', gamma= 0.5, C= 100),
    ...
    for met in ['R2', 'lcc', 'rmse', 'srcc', 'mae', 'knd']:
        main_path_save = '../output_data/'+regr_choosen + '_results_qn_' + str(n_queries) + '_nr_ch_' + str(nr_chunks) + '_' + str(
            n_initial)
