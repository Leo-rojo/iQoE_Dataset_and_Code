...
expwith_feat=np.load('../input_data/experiences_with_features.npy')
if not os.path.exists('../output_data/exp_and_scores_each_ABR'):
    os.makedirs('../output_data/exp_and_scores_each_ABR')
...
for abr in [th,bb,mpc]:
    ...
    np.save('../output_data/exp_and_scores_each_ABR/'+str(nr_c)+'_chunks_exp_'+abrnames[conta],list_of_exp)
    ...
    params_sigmoid=np.load('../input_data/save_param_sigmoids/params_sigmoid.npy')
    ...
    for u in range(32):
        ...
        for model in models:
            if model=='videoAtlas':
                with open('../input_data/'+models_folder+'/organized_by_users/user_'+str(u)+'/model_videoAtlas.pkl', 'rb') as handle:
                    synthetic_user_models.append(pickle.load(handle))
            else:
                synthetic_user_models.append(np.load('../input_data/'+models_folder+'/organized_by_users/user_'+str(u)+'/model_'+model+'.npy',allow_pickle=True))
    ...
    np.save('../output_data/exp_and_scores_each_ABR/scores_scaled_'+abrnames[conta],scores_by_models)
    np.save('../output_data/exp_and_scores_each_ABR/scores_notscaled_'+abrnames[conta],scores_by_models_not_scaled)
    conta += 1
