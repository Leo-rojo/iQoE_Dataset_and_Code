...
def each_user(nr_chunks, rs, u, model, regr_choosen,n_queries):
    ...
    for met in ['R2', 'lcc', 'rmse', 'srcc', 'mae', 'knd']:
        main_path_save = '../output_data/'+regr_choosen + '_results_qn_' + str(n_queries) + '_nr_ch_' + str(nr_chunks)+'_'+str(n_initial)+'rigs'
        ...
        for rs in [42,13,70,34,104]:
                for u in range(32):
                    for m in range(8):
                        ...
                        main_path ='../output_data/'+reg + '_results_qn_' + str(n_queries) + '_nr_ch_' + str(nr_chunk)+'_'+str(1)+'rigs'
