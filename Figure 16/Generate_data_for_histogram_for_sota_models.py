...
p1203scores=np.load('p1203_scores.npy')
bilstmscores=np.load('scoresbiqps.npy')
...
for rs in [42,13,70,34,104]:
    ...
    np.save('./sota_results/p1203_scores/mae'+str(rs),mosmodel_us_scores_mae_p1203)
    np.save('./sota_results/p1203_scores/rmse' + str(rs), mosmodel_us_scores_rmse_p1203)
    ...
    np.save('./sota_results/bilstm_scores/mae'+str(rs),mosmodel_us_scores_mae_bilstm)
    np.save('./sota_results/bilstm_scores/rmse' + str(rs), mosmodel_us_scores_rmse_bilstm)
...
for metric in ['mae','rmse']:
    for score in sc:
        each=[]
        for rs in [42,13,70,34,104]:
            each.append(np.load('./sota_results/'+score+'_scores/'+metric+str(rs)+'.npy'))
        m=np.mean(each,axis=0)
        std=np.std(each,axis=0)
        np.save('./sota_results/'+score+'_scores/'+metric+'_ave',m)
        np.save('./sota_results/' + score + '_scores/'+metric+'_std', std)