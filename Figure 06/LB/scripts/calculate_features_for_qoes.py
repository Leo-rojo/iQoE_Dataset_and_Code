...
#save scores_more_users in excel with each array a different column
import pandas as pd
df = pd.DataFrame(scores_more_users)
df.to_excel('../output_data/scores_more_users_train.xlsx', index=False)
df = pd.DataFrame(videos_more_users)
df.to_excel('../output_data/videos_more_users_train.xlsx', index=False)
...
if not os.path.exists('../output_data/features_qoes_train'):
    os.makedirs('../output_data/features_qoes_train')
    ...
    if fold.split('_')[0]=='user':
        ...
        with open(user_folder+'/Scores_test_'+identifier+'.txt') as f:
            for line in f:
               ...
               key = nextline.split()[-1]
               d_test[int(key)] = val
        scores_more_users_test.append([i for i in d_test.values()])
...
#save scores_more_users in excel with each array a different column
df = pd.DataFrame(scores_more_users_test)
df.to_excel('../output_data/scores_more_users_test.xlsx', index=False)
df = pd.DataFrame(videos_more_users_test)
df.to_excel('../output_data/videos_more_users_test.xlsx', index=False)
