...
df.to_excel('../output_data/moses.xlsx', index=False, header=False)
...
for idx_i,i in enumerate(l):
    ...
    if i == 'FTW':
        ...
        pickle.dump(fit_supreg(all_features, users_scores), open('../output_data/videoAtlas_mos.pkl', 'wb'))
    ...
    #print(i)
np.save('../output_data/params_mos',collect_all)
...
for fold in os.listdir(path_iQoE):
    if fold.split('_')[0]=='user':
        ...
        params=np.load('../output_data/params_mos.npy',allow_pickle=True)
        for kind_of_models in models:
            if kind_of_models == 'bit':  # [s_bit,s_dif_bit,s_psnr,s_dif_psnr,s_ssim,s_dif_ssim,s_vmaf,s_dif_vmaf,s_bit_log,s_dif_bit_log,ave_st_FTW,nr_stall_FTW,s_reb]
                ...
                with open('../output_data/videoAtlas_mos.pkl','rb') as handle:
...
np.save('../output_data/group_mae_each_user.npy',maes)
np.save('../output_data/group_rmse_each_user.npy',rmses)
print('-----------test_done-----------')
