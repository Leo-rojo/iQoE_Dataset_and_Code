...
regr_choosen='SVR'
#load data
group_maes=np.load('results_collection/group_mae_each_user.npy')
group_rmses=np.load('results_collection/group_rmse_each_user.npy')
pers_maes=np.load('results_collection/personalzed_literature_qoe_mae_each_user.npy')
pers_rmses=np.load('results_collection/peronalized_listerature_qoe_rmse_each_user.npy')
iqoe_maes=np.load('results_collection/iQoE_mae_each_user.npy')
iqoe_rmses=np.load('results_collection/iQoE_rmse_each_user.npy')
p1203_maes=np.load('results_collection/p1203_mae_each_user.npy')
p1203_rmses=np.load('results_collection/p1203_rmse_each_user.npy')
lstm_maes=np.load('results_collection/lstm_mae_each_user.npy')
lstm_rmses=np.load('results_collection/lstm_rmse_each_user.npy')
...
for metric in ['mae','rmse']:
    ...
    if metric=='mae':
        group_dist=group_maes.transpose()
        group_dist=group_dist.tolist()
        group_dist.append(p1203_maes)
        group_dist.append(lstm_maes)
        #group_dist=np.array(group_dist).reshape(10,nr_users)
    else:
        group_dist=group_rmses.transpose()
        group_dist=group_dist.tolist()
        group_dist.append(p1203_rmses)
        group_dist.append(lstm_rmses)
        #group_dist=np.array(group_dist).reshape(10,nr_users)

    conta = 0
    for ss in group_dist:
    ...
    # # plt.xlim(0, 25)
    # plt.show()
    plt.savefig(main_path_for_save_fig+'/'+metric+'_ECDF_dashed.pdf', bbox_inches='tight')
    plt.savefig(main_path_for_save_fig + '/' + metric + '_ECDF_dashed.png', bbox_inches='tight')
    plt.close()
...
for metric in ['mae','rmse']:
    ...
    np.save(main_path_for_save_fig+'/'+'values_ecdf_pers'+metric,save_distributions)
