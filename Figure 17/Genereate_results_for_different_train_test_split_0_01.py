...
def each_user(nr_chunks, rs, u, model, regr_choosen,n_queries,t_s):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y ,test_size=1-t_s, random_state=rs, shuffle=True)
    ...
    for idx in range(n_queries):
        ...
        if count_queries>h:
            switch=True
        query_idx, query_instance = regressor_gsio.query(X_pool_gsio,switch,rng)
...
if __name__ == "__main__":
    ...
    for nr_chunk in [7]:  # [2,4,8,
        for ts in [0.07, 0.05, 0.03, 0.01]:
            n_queries = int(ts * 1000 - 1)
